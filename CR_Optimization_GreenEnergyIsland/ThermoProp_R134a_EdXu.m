...
%        thermodynamic variables. And plot the T-s Diagram of R134a.
% Based on: MATLAB program from << Chemical, Biochemical, and Engineering
%                                  Thermodynamics >>
%          《用PR状态方程计算制冷工质的热力学性质及循环性能》
% Version: 1.0, Edward Xu, 18.4.26

function [S,H] = ThermoProp_R134a_EdXu(T,p)
...
cp3 = -1.95477E-1;
cp2 =  3.04711E-1;
eqn(4) = c_p0 == cp1 + cp2 * T + cp3 * T.^2;           % (J * mole.^-1 * K.^-1)
...
c = -6.358090E-5;
f =  6.622145E4; 
eqn(5) = c_v0 * 1000 == (a + b * T + c * T.^2 + ...
                        d * T.^3 + f ./ T.^2) ;        % J * kg.^-1 * K.^-1

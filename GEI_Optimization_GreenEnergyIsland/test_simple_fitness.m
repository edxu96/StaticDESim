...
% Version: 2.1, 2018.6.5, Jie Xu.
% Test
% SubTitle: Fitness Function for Exergy destruction Minimization
% 1. Micro Gas Turbine (MGT) with a fixed T_7 = 550K in Southern China, 1.0.
...
x(10) = 0.5322;
x(11) = 0.6711;
x(12) = 7.49047166188669;
x(13) = 0.64;
% Decision variable of ORC_R123.
x(14) = 59.1242189001039;
x(15) = 200 + 273.15;
x(16) = 5 * 101.325 * 1000;
x(17) = 40 + 273.15;
...
Cp_w = 4200;                                      % J/kg/K, water specific heat capacity at constant pressure
...
EtaG_MGTt = 0.9;
EtaG_ORCt = 0.9;                 % Efficiency of generator.
% 1.1 Constants for MGT
LHV = 500000 * 1000;                              % 纯甲烷燃料的低热值
DeltaP_MGTcc = 0.05; DeltaP_MGTreA = 0.05; ...
DeltaP_MGTreG = 0.03; DeltaP_MGThrsg = 0.05;          % 压力损失
p_1 = 101.3*1000; T_1 = 288.15;                   % 环境状态
P_8 = 500*1000; P_8p = P_8; P_9 = P_8; P_7 = P_0; % 余热锅炉水蒸气侧的热力学参数
...
EtaS_ACv = 0.99;
EtaS_ACp = 0.99;
DeltaP_ACs23 = 0.001 * 101.325 * 1000;
DeltaP_ACs45 = 0.001 * 101.325 * 1000;
...
EtaS_HPv = 0.99;                               % Isentropic efficiency of turbine.
...
EtaS_VCCv = 0.99;                                  % Efficiency of throttle valve.
EtaS_VCCp = 0.99;                                  % Efficiency of compressor(p).
...
P_3 = P_2 * (1 - DeltaP_MGTreA);                                     % (7)  RE
P_4 = P_3 * (1 - DeltaP_MGTcc);                                      % (5)  CC
P_6 = P_7 / (1 - DeltaP_MGThrsg);                                    % (14) HRSG
P_5 = P_6 / (1 - DeltaP_MGTreG);                                     % (8)  RE
T_5 = T_4 * (1 - Eta_MGTt * (1 - (P_4./P_5)^((1-Kg_MGT)/Kg_MGT))); % (9)  GT
% Calculate the h, s, E_ph of gas in status 3 (1).
...
% Calculate the h, s of air&fuel, gas in status 0.
    ...
    H_g0syms = (Hsyms - (1-Hsyms)/16*2/0.21*29) * H_a0 + ((1-Hsyms)/16*2/0.21*29) * y_H*18/MgC * H_H0 + ...
           ((1-Hsyms)/16*2/0.21*29) * y_C*44/MgC * H_C0 + ((1-Hsyms)/16*2/0.21*29) * y_N*28/MgC * H_N0;
    S_g0syms = (Hsyms - (1-Hsyms)/16*2/0.21*29) * S_a0 + ((1-Hsyms)/16*2/0.21*29) * y_H*18/MgC * S_H0 + ...
           ((1-Hsyms)/16*2/0.21*29) * y_C*44/MgC * S_C0 + ((1-Hsyms)/16*2/0.21*29) * y_N*28/MgC * S_N0;
% Calculate the h, s, E_ph of gas in status 4.
    ...
    H_g4syms = (Hsyms - (1-Hsyms)/16*2/0.21*29) * H_a4 + ((1-Hsyms)/16*2/0.21*29) * y_H*18/MgC * H_H4 + ...
           ((1-Hsyms)/16*2/0.21*29) * y_C*44/MgC * H_C4 + ((1-Hsyms)/16*2/0.21*29) * y_N*28/MgC * H_N4;
    S_g4syms = (Hsyms - (1-Hsyms)/16*2/0.21*29) * S_a4 + ((1-Hsyms)/16*2/0.21*29) * y_H*18/MgC * S_H4 + ...
           ((1-Hsyms)/16*2/0.21*29) * y_C*44/MgC * S_C4 + ((1-Hsyms)/16*2/0.21*29) * y_N*28/MgC * S_N4;
    Cp_gsyms = (H_g4syms - H_g0syms) / (T_4 - T_0);
SHsyms = solve(Hsyms == (T_4 * Cp_gsyms - Eta_MGTcc * LHV) / (H_a3 - Eta_MGTcc * LHV));     % (3)(4) CC, H = Ma / Mg;
H = double(SHsyms);
H_H0 = CoolProp.PropsSI('H', 'T', T_0, 'P', P_0, 'Water');
S_H0 = CoolProp.PropsSI('S', 'T', T_0, 'P', P_0, 'Water');
H_C0 = CoolProp.PropsSI('H', 'T', T_0, 'P', P_0, 'CarbonDioxide');
...
H_N0 = CoolProp.PropsSI('H', 'T', T_0, 'P', P_0, 'Nitrogen');
S_N0 = CoolProp.PropsSI('S', 'T', T_0, 'P', P_0, 'Nitrogen');
H_g0 = (H - (1-H)/16*2/0.21*29) * H_a0 + ((1-H)/16*2/0.21*29) * y_H*18/MgC * H_H0 + ...
       ((1-H)/16*2/0.21*29) * y_C*44/MgC * H_C0 + ((1-H)/16*2/0.21*29) * y_N*28/MgC * H_N0;
S_g0 = (H - (1-H)/16*2/0.21*29) * S_a0 + ((1-H)/16*2/0.21*29) * y_H*18/MgC * S_H0 + ...
       ((1-H)/16*2/0.21*29) * y_C*44/MgC * S_C0 + ((1-H)/16*2/0.21*29) * y_N*28/MgC * S_N0;
H_a4 = CoolProp.PropsSI('H', 'T', T_4, 'P', P_4, 'Air');
S_a4 = CoolProp.PropsSI('S', 'T', T_4, 'P', P_4, 'Air');
H_H4 = CoolProp.PropsSI('H', 'T', T_4, 'P', P_4, 'Water');
S_H4 = CoolProp.PropsSI('S', 'T', T_4, 'P', P_4, 'Water');
...
H_N4 = CoolProp.PropsSI('H', 'T', T_4, 'P', P_4, 'Nitrogen');
S_N4 = CoolProp.PropsSI('S', 'T', T_4, 'P', P_4, 'Nitrogen');
H_g4 = (H - (1-H)/16*2/0.21*29) * H_a4 + ((1-H)/16*2/0.21*29) * y_H*18/MgC * H_H4 + ...
       ((1-H)/16*2/0.21*29) * y_C*44/MgC * H_C4 + ((1-H)/16*2/0.21*29) * y_N*28/MgC * H_N4;
S_g4 = (H - (1-H)/16*2/0.21*29) * S_a4 + ((1-H)/16*2/0.21*29) * y_H*18/MgC * S_H4 + ...
       ((1-H)/16*2/0.21*29) * y_C*44/MgC * S_C4 + ((1-H)/16*2/0.21*29) * y_N*28/MgC * S_N4;
E_MGT4 = (H_g4 - H_g0) - T_0 * (S_g4 - S_g0);
Cp_g = (H_g4 - H_g0) / (T_4 - T_0);
% Calculate the H, s, E_ph of gas in status 5.
    ...
    H_g5 = (H - (1-H)/16*2/0.21*29) * H_a5 + ((1-H)/16*2/0.21*29) * y_H*18/MgC * H_H5 + ...
           ((1-H)/16*2/0.21*29) * y_C*44/MgC * H_C5 + ((1-H)/16*2/0.21*29) * y_N*28/MgC * H_N5;
    S_g5 = (H - (1-H)/16*2/0.21*29) * S_a5 + ((1-H)/16*2/0.21*29) * y_H*18/MgC * S_H5 + ...
           ((1-H)/16*2/0.21*29) * y_C*44/MgC * S_C5 + ((1-H)/16*2/0.21*29) * y_N*28/MgC * S_N5;
    E_MGT5 = (H_g5 - H_g0) - T_0 * (S_g5 - S_g0);
Mg = Wp_MGT / ((H_g4 - H_g5) - (H_a2 - H_a1)*H);                % (2) (10)
Ma = H * Mg;                                                     % H = Ma / Mg;
Mf = Mg - Ma;                                                   % (3)  CC
Ma_x = Ma - Mf/16*2/0.21*29;
T_6 = T_5 - Ma * (H_a3 - H_a2) / (Mg * Cp_g);
% H_g6 = H_g5 - Ma * (H_a3 - H_a2) / Mg;               % (6)  RE
...
H_MGT9 = H_s;
H_MGT8 = CoolProp.PropsSI('H', 'T', T_8, 'P', P_8, 'Water');
H_MGT8p = CoolProp.PropsSI('H', 'T', T_8p, 'P', P_8p, 'Water');
T_7p = T_6 - Ms * (H_MGT9 - H_MGT8p) / (Mg * Cp_g);             % (12) HRSG
T_7 = T_6 - Ms * (H_MGT9 - H_MGT8) / (Mg * Cp_g);               % (13) HRSG
Wp_MGTac = Ma * (H_a2 - H_a1);                                  % (2)  AC
Wp_MGTt = Wp_MGT + Wp_MGTac;                                    % (11) GT
We_MGT = Wp_MGT * EtaG_MGTt;
%{
H = (Cp_g * T_4 - Eta_MGTcc * LHV) / (Cp_a * T_3 - Eta_MGTcc * LHV);     % (3)(4) CC, H = Ma / Mg;
...
Ech_f = HHV;                                            % J/kg
% Calculate the h, s, E_ph of gas in status 3 (2).
H_m0 = Ma/Mg * H_a0 + Mf/Mg * H_f0;
S_m0 = Ma/Mg * S_a0 + Mf/Mg * s_f0;
H_m3 = Ma/Mg * H_a3 + Mf/Mg * H_f0;
s_m3 = Ma/Mg * S_a3 + Mf/Mg * s_f0;
...
Ed_MGTac = Wp_MGTac + Ma * E_MGT1 - Ma * E_MGT2;
Ed_MGTre = (E_MGT2 * Ma + E_MGT5 * Mg) - (E_MGT3 * Ma + E_MGT6 * Mg);
Ed_MGTcc = (E_MGT3 * Ma + Ech_f * Mf) - E_MGT4 * Mg;
...
PhiE_MGTcc = E_MGT4 * Mg / (E_MGT3 * Ma + Ech_f * Mf);
...
% Overall exergy destruction in MGT.
Ed_MGT = Ed_MGTac + Ed_MGTre + Ed_MGTcc + Ed_MGTt + Ed_MGThrsg;
% ----------------------------------------------------------------------------------------------------------------
...
P_AC1 = P_L;
Tw_AC1 = CoolProp.PropsSI('T', 'P', P_AC1, 'Q', 0, 'Water') - 273.15;
    a0_alb = -2.00755; a1_alb = 0.16976; a2_alb = -3.13336E-3; a3_alb = 1.97668E-5;
    b0_alb = 124.937;  b1_alb = -7.7162; b2_alb = 0.152286;    b3_alb = -7.9509E-4;
    T_AC1 = Tw_AC1 * (a0_alb + a1_alb * (y_1*100) + a2_alb * (y_1*100)^2 + a3_alb * (y_1*100)^3) + ...
            b0_alb + b1_alb * (y_1*100) + b2_alb * (y_1*100)^2 + b3_alb * (y_1*100)^3 + 273.15;
y_1str = num2str(y_1);
...
M_AC3 = M_AC2; y_3 = y_2;
M_AC4 = M_AC3 * y_3 / y_4;
Tw_AC4 = CoolProp.PropsSI('T', 'P', P_AC4, 'Q', 0, 'Water') - 273.15;
T_AC4 = Tw_AC4 * (a0_alb + a1_alb * (y_4*100) + a2_alb * (y_4*100)^2 + a3_alb * (y_4*100)^3) + ...
        b0_alb + b1_alb * (y_4*100) + b2_alb * (y_4*100)^2 + b3_alb * (y_4*100)^3 + 273.15;
y_4str = num2str(y_4);
...
M_AC6 = M_AC4;
E_AC6 = (H_AC6 - H_sD0) - T_0 * (S_AC6 - S_sD0);
...
Tw_AC3 = CoolProp.PropsSI('T', 'P', P_AC3, 'Q', 0, 'Water') - 273.15;
Tsat_AC3 = Tw_AC3 * (a0_alb + a1_alb * (y_3*100) + a2_alb * (y_3*100)^2 + a3_alb * (y_3*100)^3) + ...
         b0_alb + b1_alb * (y_3*100) + b2_alb * (y_3*100)^2 + b3_alb * (y_3*100)^3 + 273.15;
T_AC7 = Tsat_AC3;
...
% --------------------------------------------------------------------------------------------------------
% 5.3.1 Exergy destruction in evaporator of AC_ALB.
...
Mcw_ACe = Q_ACe / Cp_w / (T_AC17 - T_AC18);
...
% 5.3.2 Exergy destruction in desorber A_ACd. ???????????????????????????????????????????????????
...
T_AC12 = T_AC11 - Q_ACd / Cp_g / Mg;
DeltaT_ACd = ((T_AC11 - T_AC4) - (T_AC12 - T_AC3)) / log((T_AC11 - T_AC4) / (T_AC12 - T_AC3));
A_ACd = Q_ACd / DeltaT_ACd / K;
%{
...
H_AC12 = Ma_x/Mg * H_a12 + (Ma-Ma_x)/Mg * y_H*18/MgC * H_H12 + ...
       (Ma-Ma_x)/Mg * y_C*44/MgC * H_C12 + (Ma-Ma_x)/Mg * y_N*28/MgC * H_N12;
S_AC12 = Ma_x/Mg * S_a12 + (Ma-Ma_x)/Mg * y_H*18/MgC * S_H12 + ...
       (Ma-Ma_x)/Mg * y_C*44/MgC * S_C12 + (Ma-Ma_x)/Mg * y_N*28/MgC * S_N12;
E_AC12 = (H_AC12 - H_g0) - T_0 * (S_AC12 - S_g0);
...
% 5.3.3 Exergy destruction in condenser A_ACc.
...
% 5.3.6 Exergy destruction in other components of AC_ALB.
Ed_ACp = We_ACp - (E_AC2 * M_AC2 - E_AC1 * M_AC1);
PhiE_ACp = (E_AC2 * M_AC2 - E_AC1 * M_AC1) / We_ACp;
% Overall exergy destruction in AC_ALB.
Ed_AC = (Mg * E_AC11 + Mcw_ACe * E_AC17 + M_ACc * E_w0 + M_ACa * E_w0) - ...
        (Mg * E_AC12 + Mcw_ACe * E_AC18 + M_ACc * E_w0H + M_ACa * E_w0H);
% --------------------------------------------------------------------------------------------------------
...
Af_R123 = 4.643358E2; Bf_R123 =  1.625985E3; Cf_R123 = -1.333543E3;
Df_R123 = 1.986142E3; Ef_R123 = -7.172430E2;
Tr_ORC1 = T_ORC1 / Tc_R123;                                          % Reduced Temerature
D_ORC1 = Af_R123 + Bf_R123 * (1-Tr_ORC1).^(1/3) + Cf_R123 * (1-Tr_ORC1).^(2/3) + ...
         Df_R123 * (1-Tr_ORC1) + Ef_R123 - (1-Tr_ORC1).^(4/3);
V_ORC1 = 1 / D_ORC1;
...
AlphaS_VCC3 = 1 + KAPPA_R123 * (1 - sqrt(Tr_ORC3));
Alpha_VCC3 = AlphaS_VCC3^2;                                % Temp-dependent para in PR-EOS
aT_ORC3 = aTc_R123 * Alpha_VCC3;                             % Temp-dependent para in PR-EOS
...
AlphaS_VCC4 = 1 + KAPPA_R123 * (1 - sqrt(Tr_ORC4));
Alpha_VCC4 = AlphaS_VCC4^2;                                % Temp-dependent parameter in PR-EOS
aT_ORC4 = aTc_R123 * Alpha_VCC4;                             % Temp-dependent parameter in PR-EOS, on equation
...
% Exergy destruction of evaporator in ORC_R123.
T_ORC19 = T_AC12; P_ORC19 = P_AC12; P_ORC20 = P_ORC19;
H_ORC19 = H_AC12; S_ORC19 = S_AC12; E_ORC19 = E_AC12;
Q_ORCe = M_ORC * (H_ORC3 - H_ORC2);
T_ORC20 = T_ORC19 - Q_ORCe / Mg / Cp_g;
DeltaT_ORCe = ((T_ORC19 - T_ORC3) - (T_ORC20 - T_ORC2)) / log((T_ORC19 - T_ORC3) / (T_ORC20 - T_ORC2));
A_ORCe = Q_ORCe / DeltaT_ORCe / K; 
%{
syms T_ORC20 A_ORCe DeltaT_ORCe
    eq_ORC1(1) = DeltaT_ORCe == ((T_ORC19 - T_ORC3) - (T_ORC20 - T_ORC2)) / ...
                              log((T_ORC19 - T_ORC3) / (T_ORC20 - T_ORC2));
    eq_ORC1(2) = Q_ORCe == DeltaT_ORCe * K * A_ORCe;
    eq_ORC1(3) = Q_ORCe == Mg * Cp_g * (T_ORC19 - T_ORC20);
    [ST_ORC20,SA_ORC1,SDELTA_T_ORC1] = solve(eq_ORC1);
         T_ORC20 = double(ST_ORC20);
          A_ORCe = double(SA_ORC1);
    DeltaT_ORCe = double(SDELTA_T_ORC1);
%}
...
syms V_HP1sym
  ...
  Alpha_VCC4 = ALPHASqrt1^2;                                  % Temp-dependent para in PR-EOS
  aT_HP1 = aTc_R410a * Alpha_VCC4;                              % Temp-dependent para in PR-EOS
  SV_HP1sym = solve(P_HP1 == R*T_HP1 / (V_HP1sym-b_R410a) - ...
                         aT_HP1 / (V_HP1sym*(V_HP1sym+b_R410a) + b_R410a*(V_HP1sym-b_R410a)));
...
syms V_HP4sym
  ...
  AlphaS_VCC4 = 1 + Kappa_R410a * (1 - sqrt(Tr_HP4));
  Alpha_VCC4 = AlphaS_VCC4^2;                                 % Temp-dependent para in PR-EOS
  aT_ORC4 = aTc_R410a * Alpha_VCC4;                          % Temp-dependent para in PR-EOS
...
Af_R410a = 1.000000; Bf_R410a =  1.984734;    Cf_R410a = -1.767593E-01;
Df_R410a = 1.819972; Ef_R410a = -7.171684E-1;
Tr_HP3 = T_HP3 / Tc_R410a;                             % Reduced Temerature
D_HP3 = Dc_R410a * (Af_R410a + Bf_R410a * (1-Tr_HP3).^(1/3) + Cf_R410a * (1-Tr_HP3).^(2/3) + ...
                  Df_R410a * (1-Tr_HP3) + Ef_R410a - (1-Tr_HP3).^(4/3));
V_HP3 = D_HP3 / 1;
% <9> Solve for P_HP3.
syms P_HP3sym
  Tr_HP3 = T_HP3 / Tc_R410a;                             % Reduced Temerature
  AlphaS_VCC3 = 1 + Kappa_R410a * (1 - sqrt(Tr_HP3));
  Alpha_VCC3 = AlphaS_VCC3^2;                                  % Temp-dependent para in PR-EOS
  aT_ORC3 = aTc_R410a * Alpha_VCC3;                           % Temp-dependent para in PR-EOS
...
syms V_HP2sym
  ...
  AlphaS_VCC2 = 1 + Kappa_R410a * (1 - sqrt(Tr_HP2));
  Alpha_VCC2 = AlphaS_VCC2^2;                              % Temp-dependent para in PR-EOS
  aT_HP2 = aTc_R410a * Alpha_VCC2;                          % Temp-dependent para in PR-EOS
  SV_HP2sym = solve(P_HP2 == R*T_HP2 / (V_HP2sym-b_R410a) - ...
                  aT_HP2 / (V_HP2sym*(V_HP2sym+b_R410a) + b_R410a*(V_HP2sym-b_R410a)));
...
% 5.5.6 Exergy destruction in condenser of HP_R410a.
...
% 5.5.7 Exergy destruction in pump of HP_R410a.
We_HPp = M_HP * (H_HP2 - H_HP1);
Ed_HPp = We_HPp - (E_HP2 * M_HP - E_HP1 * M_HP);
...
% Overall exergy destruction of HP_R410a.
Ed_HP = (We_HPp + M_HPe * E_w0 + Ms_HPc * E_MGT8) - (M_HPe * E_w0L + Ms_HPc * E_s);
% ----------------------------------------------------------------------------------------------------
%% 5.6 Mathematical Model of VCC_R134a. -----------------------------------------------------------
% ----------------------------------------------------------------------------------------------------
Mcw_VCCe = Mcw - Mcw_ACe;
Q_VCCe = Mcw_VCCe * (H_w0 - H_cw);
% 5.6.1 Status 0 of VCC_R134a.
...
A_R134a =  4.069889E1;  B_R134a = -2.362540E3;  C_R134a = -1.306883E1;
D_R134a =  7.616005E-3; E_R134a =  2.342564E-1; F_R134a =  3.761111E2;
P_VCC1 = 10^(A_R134a + B_R134a/T_VCC1 + C_R134a * log10(T_VCC1) + D_R134a * T_VCC1 + ...
         E_R134a * ((F_R134a-T_VCC1)/T_VCC1) * log10(F_R134a-T_VCC1)) * 1000;
% <2> Solve for V_VCC1
syms V_VCC1sym
  Tr_VCC1 = T_VCC1 / Tc_R134a;                             % Reduced Temerature
  ALPHASqrt1 = 1 + Kappa_R134a * (1 - sqrt(Tr_VCC1));
  Alpha_VCC4 = ALPHASqrt1^2;                                   % Temp-dependent para in PR-EOS
  aT_VCC1 = aTc_R134a * Alpha_VCC4;                             % Temp-dependent para in PR-EOS
...
syms V_VCC4sym
  ...
  AlphaS_VCC4 = 1 + Kappa_R134a * (1 - sqrt(Tr_VCC4));
  Alpha_VCC4 = AlphaS_VCC4^2;                                   % Temp-dependent para in PR-EOS
  aT_VCC4 = aTc_R134a * Alpha_VCC4;                             % Temp-dependent para in PR-EOS
...
Af_R134a =  5.281464E2; Bf_R134a =  7.551834E2; Cf_R134a = 1.028676E3;
Df_R134a = -9.491172E2; Ef_R134a = 5.935660E2;
Tr_VCC3 = T_VCC3 / Tc_R134a;                             % Reduced Temerature
RHO_3 = Af_R134a + Bf_R134a * (1-Tr_VCC3).^(1/3) + Cf_R134a * (1-Tr_VCC3).^(2/3) + ...
        Df_R134a * (1-Tr_VCC3) + Ef_R134a - (1-Tr_VCC3).^(4/3);
V_VCC3 = RHO_3 / 1;
%{
% <9> Solve for P_VCC3.
syms P_VCC3sym
  Tr_VCC3 = T_VCC3 / Tc_R134a;                             % Reduced Temerature
  AlphaS_VCC3 = 1 + Kappa_R134a * (1 - sqrt(Tr_VCC3));
  Alpha_VCC3 = AlphaS_VCC3^2;                                 % Temp-dependent para in PR-EOS
  aT_VCC3 = aTc_R134a * Alpha_VCC3;                          % Temp-dependent para in PR-EOS
...
syms V_VCC2sym
  ...
  AlphaS_VCC2 = 1 + Kappa_R134a * (1 - sqrt(Tr_VCC2));
  Alpha_VCC2 = AlphaS_VCC2^2;                                    % Temp-dependent para in PR-EOS
  aT_VCC2 = aTc_R134a * Alpha_VCC2;                             % Temp-dependent para in PR-EOS
...
% 5.6.6 Exergy destruction in evaporator of VCC_R134a. Absorb heat through evaporator.
...
% Overall exergy destruction of VCC_R134a.
Ed_VCC = We_VCCp - (E_w0 * Mcw_VCCe - E_cw * Mcw_VCCe);
